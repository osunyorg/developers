...
Actuellement on choisit explicitement les langues disponibles dans une université parmi l'ensemble des langues disponibles sur Osuny (model `Language`). Ensuite on choisit une langue par défaut pour l'université (nécessairement parmi les langues disponibles pour cette université).
...
A la création d'un site web on choisit une liste de langues disponibles pour le site Web en fonction des langues disponibles pour l'université. Et on choisit la langue par défaut du site parmi les langues sélectionnées (on force le choix d'au moins une langue pour pouvoir avoir cette langue par défaut).
